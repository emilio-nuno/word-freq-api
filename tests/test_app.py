...
# TODO: Create data quality test suite
...
def test_build_preprocessed_query_structure():
    ...
    word_number = constants.TOP_WORDS_DEFAULT_LIMIT
    ...
    assert f"LIMIT {constants.TOP_WORDS_DEFAULT_LIMIT}" in sql
...
def test_build_unprocessed_query_different_years():
    ...
    sql_2000_2019 = _build_unprocessed_query(
        table,
        constants.TOP_WORDS_DEFAULT_LIMIT,
        constants.PROCESSED_DATA_START_YEAR,
        constants.PROCESSED_DATA_END_YEAR,
    )
    assert str(constants.PROCESSED_DATA_START_YEAR) in sql_2000_2019
    assert str(constants.PROCESSED_DATA_END_YEAR) in sql_2000_2019

    sql_1500_1600 = _build_unprocessed_query(
        table, constants.TOP_WORDS_DEFAULT_LIMIT, 1500, 1600
    )
...
def test_build_unprocessed_query_execution(in_memory_db: str):
    ...
    sql = _build_unprocessed_query(table, 5, constants.PROCESSED_DATA_START_YEAR, 2001)
    result = conn.execute(sql)
    rows = result.fetchall()
...
def test_query_limit_respected(in_memory_db: str):
    ...
    sql_unprocessed = _build_unprocessed_query(
        table_unprocessed,
        5,
        constants.PROCESSED_DATA_START_YEAR,
        constants.PROCESSED_DATA_END_YEAR,
...
def test_preprocessed_vs_unprocessed_data_difference(in_memory_db: str):
    ...
    table_unprocessed = Table(constants.UNPROCESSED_TABLE_NAME)
    sql_unprocessed = _build_unprocessed_query(
        table_unprocessed, 1, constants.PROCESSED_DATA_START_YEAR, 2001
    )
