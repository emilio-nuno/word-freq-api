...
def test_valid_start_year_invalid_end_year():
    ...
    with pytest.raises(
        ValidationError,
        match=f"Input should be less than or equal to {src_constants.RAW_DATA_END_YEAR}",
    ) as exc_info:
        CommonParams(
            start_year=src_constants.RAW_DATA_START_YEAR,
            end_year=src_constants.RAW_DATA_END_YEAR + 1,
        )
...
def test_invalid_start_year_valid_end_year():
    ...
    with pytest.raises(
        ValidationError,
        match=f"Input should be greater than or equal to {src_constants.RAW_DATA_START_YEAR}",
    ) as exc_info:
        CommonParams(
            start_year=src_constants.RAW_DATA_START_YEAR - 1,
            end_year=src_constants.RAW_DATA_END_YEAR,
        )
